...
CENTERS_DIR=./../../annotation_csv/             # Path to where csv centers are
...
printf "Results will be saved in $OUTPUT_DIR$results_csv\n\n"
...
do
    if [[ "$entry" == *.tif ]] || [[ "$entry" == *.tiff ]]; then
        ...
        # Delete the dat file sine we don't use it
        rm "$IMG_DIR$output_dat"
        # The output file is useless too so we delete it
        rm "$OUTPUT_DIR${fname%.*}_output.tif"
        ...
        output_csv="${fname}_farsight_centers.csv"
        ...
        python3 evaluate_centers.py $OUTPUT_DIR$output_csv $CENTERS_DIR$annotated_centers $OUTPUT_DIR$results_csv $fname $entry
        printf "Elapsed time: $SECONDS seconds\n\n"
    fi
done
